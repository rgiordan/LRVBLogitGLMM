...
model {
  ...
  target += 0.5 * (e_mu2 + 2 * e_mu * mu_prior_mean + mu_prior_mean^2) / mu_prior_mean;
  target += (tau_prior_alpha - 1) * e_log_tau - tau_rate * e_tau;
  target += 0.5 * (
  ...
  for (g in 1:NG) {
    // u[g] ~ normal(mu, 1 / tau);
    target += 0.5 * e_tau * (e_mu2 + 2 * e_mu * e_u[g] + (e_u[g]^2 + var_u[g]));
  }
